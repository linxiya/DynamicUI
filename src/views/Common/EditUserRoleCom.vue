  ...
  >
    <div class="transferForm">
      ...
      <el-row :gutter="20" >
        <!-- 未绑定用户列 -->
        <el-col :span="11">
          <div style="font-weight: bold;text-align: center;" >未绑定角色</div>
          <el-form
            inline
            label-width="80px"
          ...
          >
            <el-form-item label="角色编码" >
              <el-input
                maxlength="100"
                v-model="searchUnboundForm.roleCode"
            ...
            <el-form-item label="用户名称">
              <el-input
                maxlength="100"
                v-model="searchUnboundForm.roleName"
                @keyup.enter="fetchUnboundUsers"
                clearable
              ></el-input>
            </el-form-item>
        ...
        <!-- 已绑定用户列 -->
        <el-col :span="11">
          <div style="font-weight: bold;text-align: center;" >已绑定角色</div>
...
import { getList,bindUserRole } from "@/api/BasicSystem/SysRole"; // 用于获取角色列表
...
// 已绑定角色列表
const boundRoleList = ref([]);
const loadingBound = ref(false);
// 已绑定角色通常一次性加载，如果需要分页，也需要类似 paginationUnbound 的结构

// 选中项
const selectedUnboundUsers = ref([]); // 左侧选中的角色
const selectedBoundUsers = ref([]);   // 右侧选中的角色
...
const isUserSelectable = (row) => {
  return !boundRoleList.value.some(boundUser => boundUser.roleId === row.roleId);
};
...
async function fetchBoundUsers() {
  ...
  try {
    const res = await getList({
      ...
      status: 1,
      userId: currentOrg.userId,
      searchType: 1, // 假设 searchType=1 表示查询已绑定到此 userId 的用户
    });
...
function addSelectedUsers() {
  selectedUnboundUsers.value.forEach(userToAdd => {
    ...
    if (!boundRoleList.value.some(boundUser => boundUser.roleId === userToAdd.roleId)) {
      boundRoleList.value.push({ ...userToAdd }); // 添加副本
    }
  });
...
function removeSelectedUsers() {
  const idsToRemove = selectedBoundUsers.value.map(user => user.roleId);
  boundRoleList.value = boundRoleList.value.filter(user => !idsToRemove.includes(user.roleId));
  // 清空右侧选择
...
async function handleSaveBinding() {
  ...
  saving.value = true;
  const userIdsToBind = boundRoleList.value.map(user => user.roleId);
  try {
    await bindUserRole(currentOrg.userId, userIdsToBind);
...
watch(() => props.visible, (newVal) => {
  if (newVal) {
    if (props.userData && props.userData.userId) {
      currentOrg.userId = props.userData.userId;
      currentOrg.userName = props.userData.userName || '';
      currentOrg.userCode = props.userData.userCode || '';
